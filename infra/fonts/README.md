...
| `Gaegu-Regular.ttf`            | `Gaegu`              | SIL OFL 1.1 (`OFL-Gaegu.txt`)            |
| `Jua-Regular.ttf`              | `Jua`                | SIL OFL 1.1 (`OFL-Jua.txt`)              |
| `YeonSung-Regular.ttf`         | `Yeon Sung`          | SIL OFL 1.1 (`OFL-YeonSung.txt`)         |
| `NanumBrushScript-Regular.ttf` | `Nanum Brush Script` | SIL OFL 1.1 (`OFL-NanumBrushScript.txt`) |
...
개구·주아·연성·도현은 원본부터 자주 쓰는 한글 2,350자만 있다 — 드문 글자는 같은 식으로 떨어진다.

subset 명령(fontTools). 웹용은 같은 명령에 `--flavor=woff2`:

```
pyftsubset 원본.ttf --no-hinting --layout-features='*' --name-IDs='*' \
  --unicodes='U+0020-007E,U+00A0-00FF,U+2010-2027,U+2030-205E,U+20A9,U+2190-2193,U+2600-266F,U+3000-303F,U+3131-318E,U+AC00-D7A3,U+FF01-FF5E'
```

고운돋움·감자꽃도 후보였으나 subset 후에도 TTF 가 6~7MB 라(저장소와 이미지에 그대로 실린다)
뺐다 — 고딕·손글씨는 이미 있는 것과 역할이 겹친다.
