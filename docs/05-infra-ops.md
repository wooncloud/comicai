...
직전에 `.env.generated` 로 옮겨 적고(`compose.sh:79`), env-file 을 둘 넘긴다
(`compose.sh:82`, `:87`). 뒤에 온 `.env` 가 이긴다 — §5.1 과 같은 순서다.
`.env.generated` 는 커밋하지 않는다(`.gitignore`).
이때 `node` 는 PATH → `NODE_BIN` → nvm(`${NVM_DIR:-$HOME/.nvm}`, `.nvmrc` 우선) → Homebrew(`/opt/homebrew`, `/usr/local`)
순서로 찾아 비대화형 러너나 ssh 환경에서도 동작한다(`compose.sh:26-72`).
...
|                 | `STORAGE_AUTO_CREATE_BUCKET`                     | P    | `1`                                                                                   | 기동 시 버킷 자동 생성 (`storage.service.ts:79`)                                                   |
...
|                 | `EMAIL_FROM`                                     | P    | dev `ComicAI <onboarding@resend.dev>` / prod `ComicAI <no-reply@comic.wooncloud.com>` | 발신 주소 (`full.yml:38`). prod 는 인증된 도메인, dev 는 Resend 테스트 주소라 계정 주인에게만 간다 |
...
**의존 서비스 포트는 루프백에만 연다** — postgres(`full.yml:53`), redis(`:73`),
