...
| `['render-job', jobId]`      | `components/editor/panel-inspector.tsx:90`  | 단일 렌더 잡 (`qk.renderJob(jobId)`). `enabled: !!activeJobId`. SSE 이벤트가 도착할 때마다 `setQueryData`로 패치                            |
...
`InspectorShell`(`components/editor/inspector-shell.tsx:34`) — 다섯 인스펙터(컷·말풍선·
...
│   ├── project/episode-list.tsx # 화 목록(화 카드 + 그 안의 페이지)
...
### 입력칸의 보기는 **실제로 쓸 길이**로 적는다

설정집 설명칸의 안내는 "생김새·성격·분위기 등을 적어 두면 그림에 반영됩니다" 한 줄이었다.
틀린 말은 아니지만, 처음 온 사람은 그걸 읽고도 두 단어짜리 설명을 쓴다 — **얼마나
자세히** 써야 하는지가 안 적혀 있기 때문이다. 그림 품질은 거기서 갈린다.

그래서 보기를 실제로 쓸 법한 길이와 밀도로 적는다(`lib/entity-examples.ts`). 갈래마다
봐야 할 것이 다르다 — 그림체는 선과 채색, 캐릭터는 생김새와 버릇, 배경은 공간과 빛,
세계관은 규칙과 사람들의 형편. 이름·별칭 칸도 갈래별 보기를 쓴다. 새 프로젝트
다이얼로그도 같은 이유로 문구를 고쳤다(`components/dashboard/project-create-dialog.tsx`).
...
웹툰은 한 화가 끊김 없이 흐르는 한 덩어리라 페이지를 한 장씩 받으면 올릴 때 다시
이어 붙여야 한다. 인스타처럼 넘겨 보는 형식과 출판은 반대로 한 장씩이어야 한다.

- 범위: **이 페이지 한 장** / **이 화 전체**
- 화 전체일 때 방식: **세로로 이어 붙이기**(웹툰) / **페이지마다 한 장씩**(인스타·출판)
- 화 전체일 때 받기: **낱장 그대로** / **ZIP 한 개로 묶기** / **PDF 한 개로 (인쇄용)**
- 프로젝트마다 형식을 미리 정해 두지 않았다 — 같은 작품을 웹툰으로도 인스타로도
  내보내는 일이 실제로 있다. 고르는 것은 내보낼 때다.

**「받기」가 「방식」과 따로 있는 이유**는 두 질문이 다르기 때문이다. 방식은 몇 장을
만드는가이고, 받기는 그것을 어떻게 건네는가다. 인스타 20장을 낱장으로 받으면 링크를
스무 번 눌러야 하고, 출판은 인쇄로 넘기니 PNG 낱장보다 PDF 한 권이 맞다. 이어 붙인
결과가 여러 장으로 나뉜 경우에도 묶을 수 있다.

**결과는 목록으로 보여 준다**(`components/editor/export-dialog.tsx`). 예전에는
`window.open` 으로 새 탭을 열었는데, 화 하나가 여러 장이 되면 두 번째부터 팝업 차단에
걸린다. 링크를 남겨 두면 차단도 없고 받다가 놓친 것을 다시 받을 수도 있다.

빈 컷 경고는 **이 페이지 기준**이다. 화 전체는 다른 페이지의 빈 컷까지 세지 못하므로
그때는 묻지 않는다 — 틀린 숫자로 묻느니 묻지 않는 편이 낫다.

### 화(話) — 페이지를 묶는 단위

페이지가 프로젝트에 바로 매달려 한 줄로 늘어서 있었다. 단편이면 괜찮지만 연재를
시작하는 순간 40장이 평평하게 쌓여, 3화를 고치려면 목록을 세어야 했다.

- **프로젝트 화면**: 설정집 요약 아래가 화 목록이다(`components/project/episode-list.tsx:43`).
  화 카드마다 제목(인라인 편집)·페이지 수·메뉴가 있고, 접으면 제목만 남는다.
  페이지는 카드 안에 있다.
- **에디터 사이드바**: 화 > 페이지 트리(`components/editor/page-sidebar.tsx:38`).
  **지금 보고 있는 화만** 자동으로 펼친다 — 나머지는 제목만 남아 목록이 짧게 유지된다.
  사용자가 손으로 연 화는 페이지를 옮겨도 닫지 않는다.
- **브레드크럼**에 화가 한 칸 들어간다. 페이지만 보여 주면 "3화의 2쪽" 인지 "1화의
  2쪽" 인지 알 수 없다 — 페이지 이름은 화마다 다시 1쪽부터 시작한다.
...
훑은 사람은 지우는 방법이 없다고 읽는다 — 단축키는 화면 어디에도 적혀 있지 않다.
자리도 인스펙터마다 다르면 매번 찾아야 한다.

그래서 `InspectorShell` 이 `onDelete`/`deleteLabel` 을 받아 **맨 아래 같은 자리**에
그린다(`components/editor/inspector-shell.tsx:34`). 컷·말풍선·텍스트·직선이 모두 쓴다.
컷만 확인을 묻는다 — 생성 기록이 함께 사라지기 때문이다. 나머지는 Delete 키와 같게
바로 지우고, 되돌리기는 tldraw 가 받는다.

### 꼬리와 몸통은 한 덩어리로 — 선을 먼저, 바깥쪽으로

꼬리를 따로 그리면 몸통 테두리가 꼬리를 가로질러 **둘 사이에 선이 한 줄** 남는다.
만화의 말풍선은 이어진 한 덩어리다. 그래서 `BUBBLE_DRAW_ORDER`(`packages/types/src/bubble-path.ts:56`)
대로 그린다 — 꼬리·몸통의 **선을 두 배 굵기로 먼저**(채움 없이), 그 위에 두 채움을(선 없이).

SVG 선은 경계의 가운데에 걸린다. 두 배로 그린 선의 안쪽 절반을 채움이 덮으니, 선은 도형
**바깥쪽으로만** 정한 굵기만큼 남는다. 몸통 테두리가 꼬리를 지나는 구간과 꼬리 밑변도 채움에
덮여 둘 사이에 선이 남지 않는다. 예전에는 꼬리(선+채움) → 몸통(선+채움) → 꼬리(채움만) 세 번이었는데,
마지막 꼬리 채움이 꼬리 선의 안쪽 절반까지 덮어 **꼬리 선만 절반 굵기**로 보였다(2026-09-26).

캔버스(`components/editor/tldraw/speech-bubble-shape.tsx`)와 export
(`apps/api/src/export/speech-bubble.render.ts`)가 같은 순서를 지켜야 결과가 같다.
채움이 불투명할 때만 통하는 방법이다 — 색은 불투명 hex 만 받는다.

**꼬리 두께**는 꼬리가 있을 때 인스펙터의 슬라이더로 정한다(`TAIL_WIDTH_RANGE`, `bubble-path.ts:62`,
20~200%). 풍선 모양·꼬리 길이로 자동으로 정해지는 밑변 폭에 곱하는 배율이라, 자동 폭이
하던 일(넓적한 풍선에서 바늘처럼 가늘어지지 않게)은 그대로다. `shape.tail.width` 에 저장하고,
없으면 100 — 두께 조절이 생기기 전에 저장된 꼬리다.
...
- **팔레트는 속성 창에 펼쳐 둔다.** 맨 끝 줄이 지금 색과 hex 입력이다 — 팔레트에 없는 색은
  ...
  되돌림). 견본을 작게 해 한 번에 보이게 두니 누르면 끝난다.
- **직접 고르기**(채도판·색상 띠, `CustomPicker`, `color-field.tsx:181`)는 끝 줄의 스포이드 버튼으로 그 자리에서
  펼친다. 늘 펼쳐 두면 색칸이 셋인 말풍선 메뉴가 너무 길어진다. 팔레트를 펼치며 한때 뺐다가
  되살렸다 — hex 만으로는 "조금 더 밝게" 같은 고르기를 할 수 없다.
- 판·띠는 **손을 뗄 때 한 번** 알린다. 판은 포인터가 움직일 때마다 값이 나오므로, 처음에는
  페이지 배경을 한 번 끌면 `PATCH` 가 수십 개 나갔다. 캔버스 도형처럼 저장이 늦게 나가는
  대상만 `live` 를 켜서 끄는 동안에도 캔버스에 보인다.
...
- `renderErrorMessage`(`:163`) 는 워커가 실어 보내는 `RenderError.category` 를, `oauthErrorMessage`(`:188`)
  는 OAuth 콜백 쿼리 파라미터를 각각 다룬다.
- `renderCategoryMessage`(`:159`) 는 렌더 잡이 SSE 로 알려 온 실패를 분류만 보고 문장으로 바꾼다.
  **서버가 같이 보내는 `message` 는 쓰지 않는다** — 개발자용 문자열이라 `no gemini key` 가 그대로
  인스펙터 붉은 상자에 떴다(2026-09-25). 호출부는 `panel-inspector.tsx:243`.
- `adminTokenErrorMessage`(`lib/error-message.ts:141`) 는 운영자의 토큰 지급·회수 실패를 다룬다. 일반 문구를 쓰면 회수 한도 초과 시 운영자에게 "충전 후 다시 시도해 주세요" 가 나가므로, 운영자 맥락에 맞게 "회수할 수 있는 것보다 많습니다 (요청 N, 잔액 M)." 로 분기한다(`required`, `:144`).
