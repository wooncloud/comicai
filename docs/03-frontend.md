...
`components/editor/tldraw/use-shape-sync.ts:102` — 컷·말풍선·자유 텍스트·자유 직선이 **같은 코드**를
쓴다. 각 훅은 `ShapeSyncSpec`(shape type, id prop 이름, 경로 두 개, `toBody`)만 넘긴다.
...
- **실패한 저장은 큐로 되돌린다**(`:288`). 예전에는 `await` 앞에서 큐를 비워서, PATCH 가 실패해도
...
- **떠날 때 남은 편집을 보낸다**(`:340`, `flushNow`). 예전에는 정리 함수가 `clearTimeout` 만 해서,
  사이드바에서 다른 페이지를 클릭하면 방금 옮긴 위치가 서버에 한 번도 가지 않았다. `keepalive` 로
  내보내고, `beforeunload` 에서는 확인도 받는다(`:345`).
- **대기 중인 변경은 shape 스냅샷이 아니라 id 로 들고 있다**(`:117`). 스냅샷을 들면, 생성 응답으로
  서버 id 가 주입될 때(그 갱신은 `mergeRemoteChanges` 안이라 리스너가 보지 못한다) 스냅샷이 낡은
  채로 남아 "id 가 없다" 는 이유로 통째로 버려졌다.
- **되살리기는 생성이 아니라 복구다**(`:369`). 삭제를 Cmd+Z 로 되돌리면 tldraw 는 `added` 로
...
- **재조회가 편집을 덮었다.** 생성이 성공하면 서버 목록을 다시 읽어 부모에게 넘기고, 부모는 그
  목록으로 캔버스를 다시 그린다 — 즉 재조회는 캔버스를 서버 상태로 덮어쓴다. 그래서 이 훅은
  아직 못 보낸 편집이 있는 서버 id 를 `hasUnsaved` 로 노출하고(`use-shape-sync.ts:71`, `:139`),
  역방향 투영이 그 id 를 건너뛴다(`use-panel-sync.ts:68` 외 3곳 동일). 재조회를 없애는 것으로는
...
- **끝내 만들지 못한 도형이 유령으로 남았다.** 생성이 재시도를 다 쓰면 그 도형은 서버 id 가 없다.
  ...
  영영 저장되지 않는다 — 화면에는 "저장됨" 이라고 뜬 채로. 지금은 캔버스에서 지운다(`:312`).
- **포기할 때 시도된 적 없는 편집까지 버렸다.** 예전에는 재시도를 다 쓰면 큐를 통째로 비웠는데,
  마지막 왕복이 도는 사이에 들어온 편집은 한 번도 시도된 적이 없다. 지금은 실패한 것만 버리고,
  남은 것이 있으면 "저장됨" 대신 다시 예약한다(`settle`, `:249`).

증상이 전부 조용하다 — 예외도 콘솔 오류도 없고 배지는 "저장됨" 이다. 그래서 회귀해도 아무도
모른다. `use-shape-sync.spec.tsx` 가 네 시나리오를 각각 고정하고 있고, **네 개 전부 수정 전
코드에서 깨지는 것을 확인한 뒤에** 넣었다. 실제 tldraw 대신 최소 스텁을 쓴다 — 이 훅이 editor
에게 묻는 것은 `getShape`·`updateShape`·`deleteShapes`·`store.listen` 넷뿐이라, 그 이상을 흉내
내면 테스트가 tldraw 버전에 묶인다.

#### `use-panel-sync.ts` — 반대 방향

`components/editor/tldraw/use-panel-sync.ts:36` — **DTO → 캔버스** 쪽만 남았다. 이 방향은 shape 마다
...
- 저장 대기 중인 도형은 건너뛴다(`:68`). `useShapeSync` 를 **이 이펙트보다 먼저** 부르는 것도
  그래서다(`:44`) — 이펙트는 그 훅의 `hasUnsaved` 를 읽는다
- polygon은 bbox 기준 정규화 좌표로 저장/복원 (`normalizePolygonPoints`, `:147-156`)
