...
| `50-owner/`     | **운영자(사장님)가 직접 해야 할 일과 눈으로 확인할 것.** 출시 전에 여기부터 볼 것                          |
| `60-backlog.md` | **남은 일 전체** — 사람·리더·agy 담당과 진행 순서. 에이전트 규약은 루트 `AGENTS.md`                        |
| `90-decisions/` | ADR (Architecture Decision Records)                                                                        |

## 출시 준비 중이라면

먼저 [`50-owner/01-todo.md`](./50-owner/01-todo.md) 를 보라 — 지금 무엇이 막혀 있는지와
그걸 푸는 순서가 있다. 기능이 붙은 뒤에는
[`50-owner/02-verify.md`](./50-owner/02-verify.md) 로 화면을 하나씩 확인한다.
