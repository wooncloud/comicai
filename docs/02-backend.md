...
각 패널의 `currentRender` 결과를 패널 shape 마스크(SVG)로 잘라 `composite` — `export/export.service.ts:85-137`. 그 위로 말풍선(`:139-148`) → 자유 텍스트(`:150-163`) → 자유 직선(`:165-177`) 레이어가 순서대로 쌓인다. `sharp`로 캔버스(페이지 size, alpha)를 만들어 전체를 합성하며 dpi는 `withMetadata({ density: dpi })`(기본 150)로 박힌다 — `:179-190`. 결과는 S3에 `exports/{userId}/{pageId}/{ulid}.{ext}` 키로 업로드 후 presign URL 반환 — `:192-200`.

**캔버스 크기는 방어적으로 묶는다** — `clampDimension` (`export.service.ts:211`) 이 페이지 크기를
`MAX_PAGE_DIMENSION`(4096) 이하로, 패널 bounding box 도 캔버스 크기로 자른다 (`:92-93`).
