...
**원장 내역 표시는 서버가 정제한다 (`me/tokens/history`).** DB 원장(`token_ledger`)에는 운영자 내부 ID와 사유, 렌더 실패 원인 등 감사 정보가 보존되지만, 사용자에게 반환되는 DTO(`TokenLedgerEntryDTO`, `tokens.service.ts:296-318`)에서는 `memo`를 항상 `null`로 가려 네트워크 탭을 통한 정보 유출을 차단한다. 대신 `kind`, `amount`, 모델명 등 구조화된 값으로부터 정제된 화면 라벨(`label`, `ledger-label.ts:14-43`)을 단일 진실 공급원으로 제공한다: 충전은 '충전 50토큰', 운영자 조정은 '운영자 조정', 그림 생성은 '그림 생성 (Gemini)', 환급은 3종('환급 (생성 취소)', '환급 (참조 이미지 생성 실패)', '환급 (생성 실패)').
