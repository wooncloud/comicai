...
- `ModelId` — **값 목록은 `MODEL_IDS` (`src/schemas.ts:102`) 하나뿐이고** `index.ts:26` 는
  거기서 타입만 파생시킨다. 지금 고를 수 있는 것은 `SELECTABLE_MODEL_IDS`(`src/schemas.ts:118`),
  기본값은 `DEFAULT_MODEL_ID`(`src/schemas.ts:121`). 옛 판 id 는 지난 기록을 읽기 위해
...
- `PAGE_TEXT_FONT_FAMILIES` — 일반 이름 셋(`sans-serif`·`serif`·`monospace`)과 저장소에 실은 한글 글꼴 일곱 벌 (`src/schemas.ts:444`) — 캔버스(CSS)와
...
- `PAGE_LINE_STROKE_STYLES = ['solid','dashed']` (`src/schemas.ts:523`). PageLine 의 선 종류.
  폰트와 같은 이유로 값은 `schemas.ts` 에만 있고, `index.ts:363` 은 타입만 파생시킨다.
- `RenderErrorCategory = 'transient'|'auth'|'quota'|'safety'|'invalid'|'timeout'` (`src/index.ts:584`).
- `EntityType = 'style'|'character'|'background'|'worldview'` — 값은 `ENTITY_TYPES` (`src/schemas.ts:547`), 타입은 `src/index.ts:191`. `packages/db` 의 `entityIdPrefix` 도 이걸 받아서, 타입이 늘면 그 switch 가 컴파일 에러로 걸린다.
- `OAUTH_PROVIDERS = ['google','github']` (`src/index.ts:98`).
- `TEXT_ALIGNS = ['left','center','right']` (`src/schemas.ts:4`) — PageText/말풍선 인스펙터 공용 정렬 enum.
- `MODEL_LABEL`, `modelLabel(id)` — 화면 및 서버 공용 AI 모델 표시 이름('Gemini', 'OpenAI', '테스트') (`src/index.ts:749-765`).

### DTO

| DTO                                                                      | 위치                   | 주요 필드                                                                                                                                                                                                                                   |
| ------------------------------------------------------------------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ProjectDTO`                                                             | `src/index.ts:462-521` | `id, userId, name, thumbnail?(storageKey), thumbnailUrl?(presigned, 페이지 background 폴백), defaultStyleId?(대표 그림체), defaultModel?(인스펙터 모델 select 기본값), createdAt, updatedAt`                                                |
| `PageDTO`                                                                | `src/index.ts:436-454` | `id, projectId, order, name(null이면 'p{order+1}'), size{w,h}, background?, backgroundUrl?(presigned), backgroundColor?(단색)`                                                                                                              |
...
| `SpeechBubbleDTO`                                                        | `src/index.ts:267-280` | `id, pageId, variant(ellipse/rect/spike/polygon), shape{x,y,w,h,points?,tail?{x,y,width?}}, style, text, textStyle, order` — Page 직속, 패널과 독립. **대사를 직접 갖는다**(`text`·`textStyle`, 2026-09-25) — 한동안 PageText 로 분리했었다 |
...
| `PageTextDTO`                                                            | `src/index.ts:348-360` | `id, pageId, x, y, w, h, text(평문), style(PageTextStyle), order` — Page 직속 자유 텍스트 박스                                                                                                                                              |
...
| `PageLineDTO`                                                            | `src/index.ts:339-350` | `id, pageId, x1, y1, x2, y2, style(PageLineStyle), order` — Page 직속 자유 직선                                                                                                                                                             |
| `PageLineStyle`                                                          | `src/index.ts:363-367` | `strokeWidth, strokeColor, strokeStyle('solid'\|'dashed')` — `defaultPageLineStyle()` 헬퍼 (`:360-366`)                                                                                                                                     |
| `ConsistencyEntityDTO`                                                   | `src/index.ts:190-203` | `type, name, aliases[], description, refImages[], refImageUrls[](presigned), version`                                                                                                                                                       |
| `RenderJobDTO`                                                           | `src/index.ts:622-635` | `id, panelId, userId, model, status, resultImage?, resultImageUrl?(presigned), error?, attempts, finishedAt?`                                                                                                                               |
| `RenderIR`                                                               | `src/index.ts:589-610` | 워커에 전달되는 입력 IR: `styles/characters/backgrounds/worldviews`, `contiSketch?, userImages, userPrompt, aspectRatio, panelSize, seed?, outputMode?('panel'\|'entity'), systemPrompt?`                                                   |
| `RenderError`                                                            | `src/index.ts:583-587` | `category, message, rawResponse?`                                                                                                                                                                                                           |
...
| `TipTapDoc` / `TipTapNode` / `TipTapMentionAttrs`                        | `src/index.ts:381-401` | 패널 본문(mention 노드 포함). `emptyDoc()`, `flattenTipTapToText()`, `textToTipTapDoc()` 헬퍼 제공                                                                                                                                          |
...
| `TokenLedgerEntryDTO`                                                    | `src/index.ts:686-700` | `id, amount, balanceAfter, kind, label(원장 정제 라벨), refId, createdAt` — 원장 `memo` 는 내보내지 않는다(감사 정보라서)``                                                                                                                 |
...
- 화: `EpisodeCreateSchema`/`EpisodePatchSchema`/`EpisodeReorderSchema` (`src/schemas.ts:230`).
  제목은 비울 수 있고, 비면 `episodeLabel`(`src/index.ts:510`)이 순서로 "N화" 를 만든다 —
...
- 렌더: `RenderModelSchema`(ModelId enum과 동일), `RenderStartSchema` (`src/schemas.ts:286-292`).
- 내보내기: `ExportFormatSchema = 'png'|'jpg'`, `ExportRequestSchema`(dpi 72~600, 기본 150) (`src/schemas.ts:294-300`).
- 패널: `PanelShapeSchema` — points 3~64개, 좌표는 ±`MAX_PANEL_COORD`(=페이지 상한×2) 범위, strokeColor 기본 `#000000`, strokeWidth 기본 2 (`src/schemas.ts:297-345`).
- 말풍선: `SpeechBubbleVariantSchema`(4종), `SpeechBubbleShapeSchema`, `SpeechBubbleStyleSchema`(슬림 — strokeWidth/Color/fillColor 만), `SpeechBubbleCreateSchema`, `SpeechBubblePatchSchema`, `SpeechBubbleReorderSchema` (`src/schemas.ts:357-399`).
- **입력 타입은 스키마에서 파생시킨다** — `SpeechBubbleCreateInput`/`PatchInput`,
  ...
  조용히 갈라질 수 있었다.
- 페이지 텍스트: `PAGE_TEXT_FONT_FAMILIES`, `PageTextStyleSchema`, `PageTextCreateSchema`, `PageTextPatchSchema`, `PageTextReorderSchema` (`src/schemas.ts:385-418`).
- 페이지 직선: `PAGE_LINE_STROKE_STYLES`, `PageLineStrokeStyleSchema`, `PageLineStyleSchema`, `PageLineCreateSchema`, `PageLinePatchSchema`, `PageLineReorderSchema` (`src/schemas.ts:497-528`).
- 일관성: `EntityTypeSchema`, `ConsistencyCreateSchema`, `ConsistencyPatchSchema`, `ConsistencyGenerateSchema`(`prompt+model`), `ConsistencyAttachSchema`(`storageKey`) (`src/schemas.ts:538-557`).
...
const REGISTRY: Record<ModelId, ModelAdapter> = {
  ...
  // 옛 판도 같은 제공자의 지금 어댑터로 보낸다 (DB 문자열이 그대로 내려오는 경로 대비)
...
- 모델 ID: `gemini-3.1-flash-image` (`:11`) — Nano Banana 2 의 **정식판**. `-preview` 는 같은 모델이지만 예고 없이 닫힐 수 있고, 닫히면 컷이 한 장도 안 그려진다. 허용 비율은 정식판도 같다(2026-09-25, 400 응답으로 직접 확인).
...
- 에러 분류는 공통 `classifyModelHttpError` 에 위임하고, 안전성 판정만 넘긴다
  (`classifyError`, `:192`): `SAFETY:` 접두 → `safety`.
